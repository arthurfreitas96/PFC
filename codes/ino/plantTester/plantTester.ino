// Code implemented to test the plant. Time variables and interruptions are settable so it is possible to test the plant at any time.
...
const uint16_t Nsteps = 544; // 136 * 4, as the 1/4 microstep is defined
...
bool l12;
bool pulse17; // Stores pulse state
uint8_t tester;
...
void setup(void){
  ...
  pinMode(dir, OUTPUT);
  pinMode(pul, OUTPUT);
  digitalWrite(ena, HIGH); // Enables motor
  ...
  SEC = 15; // test variables
  MIN = 30;
  HR = 20;
  DAY = 12;
  MON = 10;
  YEAR = 2020;
  Serial.println(HMSToMin(HR, MIN, SEC));
  minPassed = HMSToMin(HR, MIN, SEC);
  ...
  if(minPassed < CorrSunrise){
    ...
    RTC.setAlarm(ALM1_MATCH_HOURS, SECaux, MINaux, HRaux, 0);
    Serial.println("Iniciou durante o dia. Recuperando posição.");
  }
}

void loop(void){
  if (Serial.available() > 0){
    tester = Serial.parseInt();
    if(tester == 2){ // if alarm #1 was triggered, clear its flag and execute its functions: tempRead, evalCooler, getCompassData, printDataSD.
      if(HMSToMin(HR, MIN, SEC) > (Sunset)){
        digitalWrite(ena, HIGH);
        digitalWrite(dir, LOW);
        l11 = digitalRead(limit11);
        while(l11 != 1){
    ...
    if(tester == 1){ // if alarm #2 was triggered, clear its flag and execute its functions: printTest.
      digitalWrite(ena, HIGH);
      l12 = digitalRead(limit12);
      if(l12 == 1){
        digitalWrite(ena, LOW);
        ...
        secondsToHMS(round(CorrSunriseAux * 60 + 0.5 * time_interval1Aux), &HRaux, &MINaux, &SECaux);
        RTC.setAlarm(ALM1_MATCH_HOURS, SECaux, MINaux, HRaux, 0);
        Serial.println("Chegou à chave fim de curso.");
      }
      if(time_interval1 > 60){
        MINaux = MIN + 1;
